...
class LAMA_QA_Closedbook(Dataset):
    def __init__(self, tokenizer, type_path, num_samples, input_length, output_length, print_text=False, add_all=False):
        self.dataset = load_lama(type_path, add_all) # type_path = "train", "validation", "test"
        """
        if num_samples:
            rand_indices = np.random.choice(len(self.dataset), num_samples, replace=False)
            self.dataset = self.dataset.select(list(rand_indices))
...
def get_dataset(tokenizer, type_path, num_samples, args):
    if args.dataset == "trivia":
        ...
        return Qangaroo_QA_Closedbook(tokenizer=tokenizer, type_path=type_path, num_samples=num_samples, input_length=args.max_input_length, output_length=args.max_output_length, add_all=args.add_all)
    elif args.dataset == "lama":
        return LAMA_QA_Closedbook(tokenizer=tokenizer, type_path=type_path, num_samples=num_samples, input_length=args.max_input_length, output_length=args.max_output_length, add_all=args.add_all)
    else:
        sys.exit()
